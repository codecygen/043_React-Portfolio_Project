...
const SocialMedia = (props) => {
  ...
  const socialIcons = socialMediaList.map((element) => (
    <SocialButtonCard key={element.id} link={element.link}>
      {element.icon}
    </SocialButtonCard>
  ));
  ...
  return (
    <div
      style={{
        marginTop: props.iconsMarginTop === "true" && "30px",
        marginLeft: props.iconsMarginLeft === "true" && "30px",
        marginBottom: props.iconsMarginBottom === "true" && "30px",
        marginRight: props.iconsMarginRight === "true" && "30px",
      }}
    >
      {socialIcons}
    </div>
